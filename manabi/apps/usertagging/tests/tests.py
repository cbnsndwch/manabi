...
#from ..models import Tag, UserTaggedItem
#from .models import Article, Link, Perch, Parrot, FormTest, FormTestNull
#from ..utils import calculate_cloud, edit_string_for_tags, get_tag_list, get_tag, parse_tag_input
...
#        parrots = UserTaggedItem.objects.get_by_model(Parrot, self.foo)
...
#        parrots = UserTaggedItem.objects.get_by_model(Parrot, self.bar)
...
#        parrots = UserTaggedItem.objects.get_by_model(Parrot, [self.foo, self.baz])
#        self.assertEquals(len(parrots), 0)
        
#        parrots = UserTaggedItem.objects.get_by_model(Parrot, [self.foo, self.bar])
#        self.assertEquals(len(parrots), 1)
#        self.failUnless(self.pining_for_the_fjords_parrot in parrots)
        
#        parrots = UserTaggedItem.objects.get_by_model(Parrot, [self.bar, self.ter])
#        self.assertEquals(len(parrots), 2)
#        self.failUnless(self.late_parrot in parrots)
#        self.failUnless(self.passed_on_parrot in parrots)
        
#        # Issue 114 - Intersection with non-existant tags
#        parrots = UserTaggedItem.objects.get_intersection_by_model(Parrot, [])
#        self.assertEquals(len(parrots), 0)
    
#    def test_get_by_model_with_tag_querysets_as_input(self):
#        parrots = UserTaggedItem.objects.get_by_model(Parrot, Tag.objects.filter(name__in=['foo', 'baz']))
#        self.assertEquals(len(parrots), 0)
        
#        parrots = UserTaggedItem.objects.get_by_model(Parrot, Tag.objects.filter(name__in=['foo', 'bar']))
#        self.assertEquals(len(parrots), 1)
#        self.failUnless(self.pining_for_the_fjords_parrot in parrots)
        
#        parrots = UserTaggedItem.objects.get_by_model(Parrot, Tag.objects.filter(name__in=['bar', 'ter']))
#        self.assertEquals(len(parrots), 2)
#        self.failUnless(self.late_parrot in parrots)
#        self.failUnless(self.passed_on_parrot in parrots)
    
#    def test_get_by_model_with_strings_as_input(self):
#        parrots = UserTaggedItem.objects.get_by_model(Parrot, 'foo baz')
#        self.assertEquals(len(parrots), 0)
        
#        parrots = UserTaggedItem.objects.get_by_model(Parrot, 'foo bar')
#        self.assertEquals(len(parrots), 1)
#        self.failUnless(self.pining_for_the_fjords_parrot in parrots)
        
#        parrots = UserTaggedItem.objects.get_by_model(Parrot, 'bar ter')
#        self.assertEquals(len(parrots), 2)
#        self.failUnless(self.late_parrot in parrots)
#        self.failUnless(self.passed_on_parrot in parrots)
...
#        parrots = UserTaggedItem.objects.get_by_model(Parrot, 'argatrons')
#        self.assertEquals(len(parrots), 0)
    
#    def test_get_union_by_model(self):
#        parrots = UserTaggedItem.objects.get_union_by_model(Parrot, ['foo', 'ter'])
...
#        parrots = UserTaggedItem.objects.get_union_by_model(Parrot, ['bar', 'baz'])
#        self.assertEquals(len(parrots), 3)
#        self.failUnless(self.late_parrot in parrots)
#        self.failUnless(self.passed_on_parrot in parrots)
#        self.failUnless(self.pining_for_the_fjords_parrot in parrots)
...
#        parrots = UserTaggedItem.objects.get_union_by_model(Parrot, [])
#        self.assertEquals(len(parrots), 0)

#class TestGetRelatedUserTaggedItems(TestCase):
#    def setUp(self):
#        parrot_details = (
#            ('pining for the fjords', 9, True,  'foo bar'),
#            ('passed on',             6, False, 'bar baz ter'),
...
#        related_objects = UserTaggedItem.objects.get_related(self.l1, Link)
...
#        related_objects = UserTaggedItem.objects.get_related(self.l4, Link)
...
#        related_objects = UserTaggedItem.objects.get_related(self.l1, Link, num=1)
#        self.assertEquals(len(related_objects), 1)
#        self.failUnless(self.l2 in related_objects)
        
#    def test_get_related_objects_of_same_model_limit_related_items(self):
#        related_objects = UserTaggedItem.objects.get_related(self.l1, Link.objects.exclude(name='link 3'))
#        self.assertEquals(len(related_objects), 1)
#        self.failUnless(self.l2 in related_objects)
    
#    def test_get_related_objects_of_different_model(self):
#        related_objects = UserTaggedItem.objects.get_related(self.a1, Link)
